...
-- together with the customer's cumulative spending
-- within the month when the order was placed.
-- The cumulative total should include
-- the current order.
-- Return:
-- order_id,
-- customer_id,
-- order_date,
-- order_value,
-- customer_monthly_running_total
...
(
    ...
    sum(ov.order_value) over(
        ...
        ) as customer_monthly_running_total
...
caom.order_date,
caom.order_value,
caom.customer_monthly_running_total
